...
public class PanelSchedule extends JPanel {
	...
	/**
	 * Create the panel showing the individual schedules
	 */
	public PanelSchedule(Bizrain br, Schedule sch, PanelScheduleRet ret) {
		...
		btnSchedules.setIcon(
				new ImageIcon(PanelSchedule.class.getResource("/uk/ac/cam/bizrain/ui/ico/fa-chevron-left-16.png")));
	...
	/*
	 * Redraws the schedule list if any item changes
	 */
	public void reschedule(Bizrain br, JScrollPane pan, Schedule sch, LocalTimeToEpoch lt2e) {
		PanelSchedule beme = this;

		IWeatherData iwd = br.sm.getScheduleCombinedWeather(sch, br.weatherProv, false, LocalTimeToEpoch.getDefault());
		if (pnOverview != null) {
			remove(pnOverview);
		}
